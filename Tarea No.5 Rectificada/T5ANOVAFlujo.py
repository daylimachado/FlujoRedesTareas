# -*- coding: utf-8 -*-
"""
Created on Mon Apr 29 23:58:05 2019
...
df = pd.read_csv("CSVunido.csv", index_col=None,usecols=[6,7,8,9,10,11,12],dtype={'Flujomaximo':np.int,
                                                                                   'DistGrado': np.int,
...
logX = np.log1p(df['Flujomaximo'])
df = df.assign(logflujomax=logX.values)
df.drop(['Flujomaximo'], axis= 1, inplace= True)

anova_factores=["DistGrado","CoefAgrup","CentCercania","CentCarga","ExcentCarga","PageR" ]
plt.figure(figsize=(8, 10))
for i in anova_factores:
    
    print(rp.summary_cont(df['logflujomax'].groupby(df[i])))

    anovaporfactor = pg.anova (dv='logflujomax', between=i, data=df, detailed=True , )
    pg._export_table (anovaporfactor,("Imagenes/Tabla_ANOVAflujomax"+i+".csv"))

    ejes=sns.boxplot(x=df["logflujomax"], y=df[i], data=df, palette="Set2")
    plt.savefig("Imagenes/aboxplotflujomax"+ i+".png", bbox_inches='tight')
    plt.savefig("Imagenes/aboxplotflujomax" + i + ".eps", bbox_inches='tight')
    tukey = pairwise_tukeyhsd(endog = df["logflujomax"],     # Data
                          groups= df[i],   # Groups
...
#    plt.vlines(x=49.57,ymin=-0.5,ymax=4.5, color="red")
    ...
    plt.savefig("Imagenes/tablatukeyflujomax"+ i+".png", bbox_inches='tight')
    plt.savefig("Imagenes/tablatukeyflujomax" + i + ".eps", bbox_inches='tight')
    print(tukey.summary())
    
    excel_tukey = open("Imagenes/Tukeyflujomax"+i+".csv", 'w')
    with excel_tukey:
        writer = csv.writer(excel_tukey)
        writer.writerows(tukey.summary())

#model_name = ols('logflujomax ~ generador+algoritmo_flujo+vertices+convlogdensidad +generador*algoritmo_flujo+algoritmo_flujo*vertices+vertices*convlogdensidad+generador*vertices+generador*convlogdensidad+algoritmo_flujo+vertices*convlogdensidad', data=df).fit()
#model_name.summary()
